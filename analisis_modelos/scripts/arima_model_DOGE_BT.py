...
def eval_model(modelo_arimax, fec_ini, fec_fin):
    ...
    df_doge_symbol24 = df_final24[df_final24['SYMBOL'] == 'XDGUSD'].copy()

    # 2. Renombrar la columna 'CLOSE' a 'label' para usarla más adelante
    df_doge_symbol24 = df_doge_symbol24[['DATE', 'CLOSE']].rename(columns={'CLOSE': 'label'})

    # 3. Realizar un merge con el df_final, usando la columna DATE para emparejar los valores
    df_doge24 = pd.merge(df_final24, df_doge_symbol24, on='DATE', how='left')

    # 4. Filtrar de nuevo si solo quieres quedarte con los registros de DOGE
    df_doge24 = df_doge24[df_doge24['SYMBOL'] != 'XDGUSD'].reset_index(drop=True)

    # 1. Usar pivot_table para transformar el DataFrame
    df_pivot_doge24 = df_doge24.pivot_table(index='DATE', columns='SYMBOL', values='CLOSE')

    # 2. Renombrar las columnas para que sean más claras
    df_pivot_doge24.columns = [f'{col}_CLOSE' for col in df_pivot_doge24.columns]

    # 3. Añadir la columna 'label' al DataFrame df_doge
    df_labels24 = df_doge24[['DATE', 'label']].drop_duplicates()

    # 4. Hacer un merge para combinar los datos pivotados con la columna 'label'
    df_pivot_doge24 = df_pivot_doge24.reset_index()
    df_final_doge24 = pd.merge(df_pivot_doge24, df_labels24, on='DATE', how='left')

    # 5. Rellenar valores faltantes usando backward fill
    df_final_doge24 = df_final_doge24.fillna(method='bfill')

    df_final_doge24["DATE"] = pd.to_datetime(df_final_doge24["DATE"])  # Convertir a datetime si no se ha hecho
    df_final_doge24.set_index("DATE", inplace=True)  # Asegurar que DATE es el índice

    # Filtrar registros hasta el 1 de SEPT de 2024
    df_filtrado24 = df_final_doge24.loc[:'2024-09-01']
    ...
    exog_values = df_filtrado24.loc[:, ["XBTUSD_CLOSE", "SP500_CLOSE", "GCUSD_CLOSE", "Nasdaq_CLOSE", "USDEUR_CLOSE"]]
